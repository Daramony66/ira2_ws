...
public:
  HapticControl()
  : Node("haptic_control"), rtde_control("192.168.1.101"), rtde_receive("192.168.1.101"), gripper("192.168.1.101", 63352, true), //Modifié "true" le 04/03 à 12h13
...
private:
  ...
  bool in_workspace(const std::array<double,6>& pose)
  {
    ...
    const double z_min = 0.012, z_max = 0.38;
  ...
  void haptic_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
  {
    ...
    const double alpha = 0.85; // Modifié "0.7" le 03/03 à 14h30
...

    {
      ...
      if (btn == 1) {
        ...
        const double scaling_factor = 1.5; // Modifié "3.0" le 03/03 à 14h30
