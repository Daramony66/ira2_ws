...
//#include <ur_rtde/robotiq_gripper.h> //Commenté le 23/03 à 16h32
#include <thread>
#include <chrono>
...
public:
  HapticControl()
  : Node("haptic_control"), rtde_control("192.168.1.101"), rtde_receive("192.168.1.101"), //gripper("192.168.1.101", 63352, true), //Modifié "true" le 04/03 à 12h13 // Commenté le 23/03 à 16h32
    velocity(0.1), acceleration(0.1), dt(1.0/125), lookahead_time(0.07), gain(500), //Modifié lookahead_time(0.07) le 17/03 à 15h43
    ...
    ref_ori_robot_({0.0, 0.0, 0.0}), ref_pos_robot_({0.0, 0.0, 0.0}), //gripper_closed(false), //Commenté le 23/03 à 16h32
    ...
    // bias_ready_(false), bias_collecting_(true), force_bias_({0.0, 0.0, 0.0})
  {
    ...
    // Ajouté le 19/03 à 17h50
    haptic_stiffness_pub_ = this->create_publisher<std_msgs::msg::Float64>("/haptic_stiffness", 10);

    //Ajouté le 23/03 à 17h28
    haptic_damping_pub_ = this->create_publisher<std_msgs::msg::Float64>("/haptic_damping", 10);
  ...
  ~HapticControl()
  {
    //gripper.disconnect(); //Commenté le 23/03 à 16h32
...
private:
  ...
  void haptic_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
  {
    ...
    //const double alpha = (current_force_magnitude_ > 15.0) ? 0.92 : 0.85; //Ajouté le 19/03 à 13h08
...

    {
      ...
      if (btn == 1) {
        ...
        target = {delta_position[0] * scaling_factor + ref_pos_robot_[0],
                  ...
                  /*delta_orientation[0]*0.04 + */ ref_ori_robot_[0], //Commenté le 23/03 à 16h32
  ...
  void force_callback(const geometry_msgs::msg::WrenchStamped::SharedPtr msg)
  {
    ...
    //Ajouté le 19/03 à 17h53
    
    // ETAPE 5 : EWRLS estimation Kelvin-Voigt - Ajouté le 19/03
    double z_tcp = tcp_pose[2];
    double fe = current_force_magnitude_;

    // Détection premier contact
    if (fe > 5.0 && !contact_detected_) {
        z_contact_ = z_tcp;
        contact_detected_ = true;
        prev_z_tcp_ = z_tcp;
    }

    // Reset si plus de contact
    if (fe < 2.0) {
        contact_detected_ = false;
    }

    if (contact_detected_ && fe > 5.0) {
        double x = z_contact_ - z_tcp;          // pénétration
        double x_dot = (z_tcp - prev_z_tcp_) * 500.0; // vitesse (500Hz)
        prev_z_tcp_ = z_tcp;

        if (x > 0.0001) { // seulement si pénétration réelle
            // Vecteur régression phi = [x, x_dot]
            double phi0 = x;
            double phi1 = x_dot;

            // Gain EWRLS : L = P*phi / (lambda + phi^T*P*phi)
            double denom = lambda_ + phi0*(P00_*phi0 + P10_*phi1) + phi1*(P01_*phi0 + P11_*phi1);
            double L0 = (P00_*phi0 + P01_*phi1) / denom;
            double L1 = (P10_*phi0 + P11_*phi1) / denom;

            // Mise à jour paramètres
            double error = fe - (K_est_*phi0 + B_est_*phi1);
            K_est_ += L0 * error;
            B_est_ += L1 * error;

            // Clamp pour stabilité
            K_est_ = std::max(0.001, std::min(K_est_, 10.0));
            B_est_ = std::max(0.0,   std::min(B_est_, 1.0));

            // Mise à jour matrice P
            double P00_new = (P00_ - L0*(P00_*phi0 + P01_*phi1)) / lambda_;
            double P01_new = (P01_ - L0*(P10_*phi0 + P11_*phi1)) / lambda_;
            double P10_new = (P10_ - L1*(P00_*phi0 + P01_*phi1)) / lambda_;
            double P11_new = (P11_ - L1*(P10_*phi0 + P11_*phi1)) / lambda_;
            P00_ = P00_new; P01_ = P01_new;
            P10_ = P10_new; P11_ = P11_new;

            // Publier K estimé
            std_msgs::msg::Float64 stiffness_msg;
            stiffness_msg.data = K_est_;
            haptic_stiffness_pub_->publish(stiffness_msg);

            //Ajouté le 23/03 à 17h28
            std_msgs::msg::Float64 damping_msg;
            damping_msg.data = B_est_;
            haptic_damping_pub_->publish(damping_msg);

            RCLCPP_INFO(this->get_logger(), "EWRLS - x:%.4f Fe:%.2f K:%.4f B:%.4f",
                x, fe, K_est_, B_est_);
        }
    }
  ...
  //RobotiqGripper gripper; //Commenté le 23/03 à 16h32
  int button_pressed_;
  //bool gripper_closed; //Commenté le 23/03 à 16h32
  int prev_btn_;
  bool first_position;
  // Positions
  ...
  // EWRLS - Ajouté le 19/03 à
  double z_contact_ = 0.0;
  bool contact_detected_ = false;
  double K_est_ = 0.05;
  double B_est_ = 0.01;
  
  double P00_ = 1.0, P01_ = 0.0, P10_ = 0.0, P11_ = 1.0;
  double lambda_ = 0.995;
  double prev_z_tcp_ = 0.0;
  rclcpp::Publisher<std_msgs::msg::Float64>::SharedPtr haptic_stiffness_pub_;

  rclcpp::Publisher<std_msgs::msg::Float64>::SharedPtr haptic_damping_pub_; //Ajouté le 23/03 à 17h28
...
int main(int argc, char *argv[])
{
  ...
  while (rclcpp::ok()) {
    try {
      ...
      RCLCPP_WARN(rclcpp::get_logger("main"), "Echec: %s — nouvelle tentative dans 10s...", e.what());
      if (g_node) g_node.reset();
